use crate::client::{identity_in, init_client};
...
enum Command {
    ...
    /// Move a library from before the format directories into them
    Migrate {
        /// Move the files. Without this the command only says what it would move.
        #[clap(long)]
        apply: bool,
    },
...
impl Summary {
    ...
    fn render(&self) -> String {
        ...
        if !self.changed.is_empty() {
            report += &format!("\n{}\n", style(format!("CHANGED ON BANDCAMP ({}). Previous copies are in {}/:", self.changed.len(), VERSIONS_DIRNAME)).red().bold());
        ...
        if !self.failed.is_empty() {
            report += &format!("\n{}\n", style(format!("Failed after the retries ({}). Run the tool again for these:", self.failed.len())).yellow());
        ...
        if !self.relink.is_empty() {
            report += &format!("\n{}\n", style(format!("NEED A NEW LINK ({}). Open each page and request a link by email,\nthen run the tool again:", self.relink.len())).yellow().bold());
...
async fn download_to_part<S>(position: &str, part: &PathBuf, destination: &PathBuf, filesize: usize, content_length: Option<u64>, resumed_from: u64, stream: &mut S) -> Result<(), Box<dyn Error>>
where
    S: Stream<Item=Result<Bytes, reqwest::Error>> + Unpin,
{
    ...
    match resumed_from > 0 {
        true => println!("{} {} {}", style(&position).dim(), style("continue").yellow(), name),
        _ => println!("{} {} {}", style(&position).dim(), style("download").green().bold(), name),
    }

    pb.set_style(
        ProgressStyle::default_bar()
            .template("  {spinner:.green} [{bar:38.cyan/blue}] {percent:>3}%  {bytes:>10}/{total_bytes:<10} {binary_bytes_per_sec:>11}  eta {eta:>4}")?
...
async fn download_file<S>(position: &str, destination: &PathBuf, filesize: usize, content_length: Option<u64>, resumed_from: u64, stream: &mut S) -> Result<PathBuf, Box<dyn Error>>
where
    S: Stream<Item=Result<Bytes, reqwest::Error>> + Unpin,
{
    ...
    download_to_part(position, &part, destination, filesize, content_length, resumed_from, stream).await?;
...
async fn download(options: &Options, filters: &Filters, root: &PathBuf) -> Result<i32, Box<dyn Error>> {
    let (client, jar) = init_client(&resolve_identity(options)?);
    let mut collection_iterator = CollectionIterator::new(&client, options.format);
    let mut summary = Summary::default();

    // Recorded so a change during the run is visible. This is a hash and not the
    // cookie, because the cookie logs in to the account.
    let started_with = identity_in(&jar);
    if let Some(fingerprint) = &started_with {
        println!("Identity cookie: #{}", fingerprint);
    }
    ...
    loop {
        ...
        // Bandcamp can send a new identity cookie at any time. The client stores it,
        // thus a change here says that the tool is now logged in as something else.
        if identity_in(&jar) != started_with {
            println!("The identity cookie changed during this run.");
        }
        ...
        if let Some(id) = &sale_item_id.clone().filter(|_| !options.recheck) {
            if let Some(record) = manifest.get(id, format) {
                if check_if_exists(&root.join(&record.filename)) {
                    println!("{} {} {}", style(&position).dim(), style("have").blue(), record.filename);
                    summary.skipped += 1;
        ...
        let download_url = match with_retry("Preparing archive", move || async move {
            ...
            Err(e) => {
                eprintln!("{} {} {}: {}", style(&position).dim(), style("failed").red().bold(), label, e);
        ...
        // The server gives a flac and an mp3 the same name, thus one format would
        // look like a change to the other. Each format gets its own directory.
        let held = format!("{}/{}", format, item_info.filename);
        let destination = root.join(&held);

        if let Some(parent) = destination.parent() {
            if let Err(e) = create_dir_all(parent) {
                eprintln!("Cannot make the directory for {}: {}", &item_info.filename, e);
                continue;
            }
        }

        // A library from before the directories has its files at the top level. Use
        // one that is there rather than fetching it again.
        let legacy = root.join(&item_info.filename);
        let (destination, held) = match !check_if_exists(&destination) && check_if_exists(&legacy) {
            true => (legacy, item_info.filename.clone()),
            _ => (destination, held),
        };
        ...
        if !options.recheck && !known && check_if_exists(&destination) {
            match fingerprint(&destination, &item_info.filename) {
                Ok(existing) if is_trustworthy(&destination, &existing, item_info.filesize) => {
                    println!("{} {} {}", style(&position).dim(), style("adopt").magenta(), &item_info.filename);
                ...
                Ok(_) => println!("{} {} {} is not complete", style(&position).dim(), style("redo").yellow(), &item_info.filename),
        ...
        let part = match with_retry("Download", move || async move {
            // Ask for the url again on each attempt. "It is not ready" says that
            // Bandcamp is still building the archive, and the answer to that is a new
            // url once it is built. The same url stays not ready for ever.
            let download_url = item.get_download_url(client).await?;
            ...
            download_file(position, destination, filesize, content_length, resumed_from, &mut stream).await
        }).await {
            Ok(part) => part,
            Err(e) => {
...
async fn list(options: &Options, filters: &Filters, json: bool) -> Result<i32, Box<dyn Error>> {
    let (client, _jar) = init_client(&resolve_identity(options)?);
    let mut collection_iterator = CollectionIterator::new(&client, options.format);
...
/// Move a library whose files are at the top level into the format directories.
///
/// A move inside one filesystem only changes the name, thus this is quick even for a
/// large library. The record is written after the files, thus a move that stops part
/// way leaves a record that still describes where each file is.
fn migrate(root: &PathBuf, apply: bool) -> Result<i32, Box<dyn Error>> {
    let manifest_path = root.join(MANIFEST_FILENAME);
    let mut manifest = Manifest::load(&manifest_path)?;
    ...
    for (id, item) in &manifest.items {
        for (format, record) in &item.downloads {
            // A record that already holds a directory was written after the change
            if record.filename.contains('/') {
                continue;
            }

            match check_if_exists(&root.join(&record.filename)) {
                true => planned.push((id.clone(), format.clone(), record.filename.clone(),
                                      format!("{}/{}", format, record.filename))),
                _ => absent.push(format!("{} [{}]", record.filename, format)),
            }
        }
    }

    if planned.is_empty() && absent.is_empty() {
        println!("Every file is already in a format directory.");
        return Ok(0);
    }

    println!("{} file(s) to move", planned.len());

    if !apply {
        for (_, _, from, to) in planned.iter().take(5) {
            println!("  {} -> {}", from, to);
        }

        if planned.len() > 5 {
            println!("  and {} more", planned.len() - 5);
        }

        println!("\nRun it again with --apply to move them.");
        return Ok(0);
    }

    let mut moved = 0;

    for (id, format, from, to) in planned {
        ...
        match rename(&source, &target) {
            Ok(()) => {
                if let Some(record) = manifest.items.get_mut(&id).and_then(|i| i.downloads.get_mut(&format)) {
                    record.filename = to;
                }

                moved += 1;
            }
            Err(e) => eprintln!("Cannot move {}: {}", from, e),
        }
    }

    manifest.save(&manifest_path)?;

    println!("Moved {} file(s). The record now says where each one is.", moved);

    if !absent.is_empty() {
        println!("\nIn the record but not on the disk ({}):", absent.len());
        for entry in &absent {
            println!("  {}", entry);
        }
    }
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let code = match &cli.command {
        ...
        Some(Command::Migrate { apply }) => migrate(&root, *apply)?,

...
impl<'a> traits::CollectionIterator<'a, CollectionItem> for CollectionIterator<'a> {
    ...
    async fn get_next_item(&mut self) -> Result<Option<CollectionItem>, Box<dyn Error>> {
        if self.queued.is_none() {
            ...
            // item_count leaves out the items that you hid, but the tool downloads them,
            // thus the count must include them or the run ends past its own total
            self.total = Some(collection_data.item_count + collection_data.hidden_items_count);
